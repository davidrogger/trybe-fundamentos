...
const pay = () => {
  const newMenu = restaurant.fetchMenu(); // Objeto com os itens do menu criados
  const allOrders = restaurant.consumption; // Lista com todos itens que foram solicitados na ordem
  const allKeysMenu = Object.keys(newMenu); // Lista com todas as chaves do menu
  const allItemsMenu = allKeysMenu.map((itensMenu) => newMenu[itensMenu]); // Lista com os valores dentro do menu.
  // Usando a lista de pedidos, mapiei os preços, conferindo todas as chaves, caso aquela item não exista naquela chave, ele volta como 0.
  const allCost = allOrders.map((itemOrder) => allItemsMenu.map((itemMenu) => {
    if (itemMenu[itemOrder] !== undefined) {
      return itemMenu[itemOrder];
    }
    return 0;
  }));
  // deixando tudo em um array só, para conseguir somar todos os valores usando reduce depois.
  const oneLineCost = allCost.reduce((listCost, cost) => [...listCost, ...cost], []);
  const sumCost = oneLineCost.reduce((sumCosts, cost) => sumCosts + cost, 0);
  return sumCost;
};
...
// const meuRestaurante = createMenu({ food: { coxinha: 3.90, sanduiche: 9.90 }, drink: { agua: 3.90, cerveja: 6.90 }, dessert: { icecream: 5.00 } });
// meuRestaurante.order('coxinha');
// meuRestaurante.order('agua');
// meuRestaurante.order('coxinha');
// meuRestaurante.order('coxinha');
// meuRestaurante.order('cerveja');
// meuRestaurante.order('icecream');
